"""BitNet-style MLP for encoding atlas (v0_6)."""
...
class BitNetEncodingMLP(nn.Module):
    def __init__(
        self,
        *,
        hidden_dim: int = 128,
        ...
        self.linears.append(
            EncodingLinear(
                in_dim, 10, n_bits=n_bits, encoding=encoding, n_exp=n_exp
            )
        )
    ...
    def weight_stats(self) -> dict:
        """Rough use-of-scale diagnostics for NOTES."""
        ws = []
        es = []
        for layer in self.linears:
        ...
        if es:
            ...
            out["frac_exp_extreme"] = float(
                ((e_all == 0) | (e_all == e_all.max())).float().mean().item()
            )
        return out
